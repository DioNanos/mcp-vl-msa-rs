...
| Question | Answer we measured |
|---|---|
| BM25 or dense/hybrid? | **BM25.** Three pre-registered attempts; no hybrid config beat the gate on our workloads. Dense rerank stays available (`dense_alpha`, off by default) for re-testing as encoders improve. |
| Should retrieval boost recent documents? | **No.** Recency priors lost even on their own target category. Handle time at serving, not in the score. |
| Does original-text injection (`msa_fetch_doc` after `msa_search`) matter? | **Yes, where it counts**: +14.6 F1 exactly on the stratum where snippets miss the content. Cheap insurance — keep using it. |
| Biggest retrieval win we found? | **Rich capsules at ingestion** (deterministic enrich, no LLM): +7 to +20 recall@5 across every category. |
| Does any answering scaffold fix multi-session aggregation? | **No — replicated across 4 model families** (GLM, DeepSeek, Kimi, MiniMax). The multi-session reading gap (~0.3 F1 vs single-session) is in how cross-session evidence is *represented*, not in the reader. |
...
**Replicated across model families.** The original gate used a single reader
(GLM-5.1). We re-ran the identical harness on three more families via
independent endpoints — same evidence, same prompts, temperature 0, same
scoring, same seeded sample (LongMemEval-S, 50 questions per category). One
parameter differs across families: reasoning-class readers run with a larger
token cap (4096 vs 512) so the thinking block cannot truncate the answer; the
cap is condition-constant within each reader, so every extract-vs-direct
comparison stays controlled. CIs are paired bootstrap (1000 resamples),
regenerated by `crates/msa-bench/scripts/readerstep_family_cis.py` — the
published numbers are defined by that script.

| Reader | Δ extract−direct (multi-session) | CI95 |
|---|---|---|
| GLM-5.1 | −0.000 | [−0.08, +0.08] |
| DeepSeek-v4-pro | +0.009 | [−0.06, +0.08] |
| Kimi-k2.6 | −0.018 (no-empty sensitivity: +0.002) | [−0.08, +0.05] |
| MiniMax-m3 | +0.009 | [−0.06, +0.09] |

Four families, four zeros against a +10 gate. MiniMax is the family that
ships an *intrinsic*, in-model MSA architecture — even that reader does not
close the gap from flat evidence. Methodological note: reasoning-class readers
need a generous, condition-constant token budget — at 512 tokens the thinking
block starved the answer (22–46% empty predictions) and produced a spurious
"extract hurts" delta that vanished at 4096. We discarded that run rather than
publish it. Frontier readers (Claude, GPT) are not yet tested — future work.
...
consolidation, and reader-step scaffolding. The reading gap itself (F1 ≈ 0.85
single-session vs ≈ 0.47–0.52 multi-session, ~0.3 apart) replicates on every
reader family we tested — GLM, DeepSeek, Kimi and MiniMax — with retrieval
held constant. The gap is not *how we retrieve*, not *how we ask the model to
answer*, and not *which model reads*; it is plausibly a problem of how the
cross-session evidence is *represented* before the reader. That is the open
question — it needs a new hypothesis, not another serving variant.
